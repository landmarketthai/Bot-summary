...
const MIGRATION = join(ROOT, "supabase", "migrations", "20260926090000_manual_slip_atomic_append_close.sql");
...
if (!pgAvailable && process.env.REQUIRE_MANUAL_SLIP_ATOMIC_POSTGRES === "1") {
  throw new Error("PostgreSQL 17 manual-slip atomic harness is unavailable");
}
let created = false;
